  ...
  return (
    <div className="bg-white w-full p-3 flex items-center justify-start rounded-lg shadow">
      ...
      <div className="h-20 flex flex-col justify-between items-end">
        <div onClick={() => cartDispatch({ type: "Delete_From_Cart", id: product.state.id })}>
          <FiTrash2 className="text-orange-500 text-xl cursor-pointer" />
        </div>
        <div className="flex items-center gap-x-3 select-none">
          <span onClick={() => cartDispatch({ type: "Increment", id: product.state.id })} className="bg-gray-300 text-lg cursor-pointer w-6 h-6 flex justify-center items-center text-slate-800 rounded-full"> + </span>
          <p>{product.quantity}</p>
          <span onClick={() => cartDispatch({ type: "Decrement", id: product.state.id })} className="bg-orange-300 text-lg cursor-pointer w-6 h-6 flex justify-center items-center text-white rounded-full"> - </span>
        </div>
      </div>
    </div>
  );
