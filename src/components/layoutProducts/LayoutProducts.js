          ...
          <article className="w-full text-sm md:text-base py-2 md:py-0 z-30 md:z-10 md:py-auto fixed top-[105px] md:top-auto bg-white md:bg-transparent h-[70px] md:h-auto md:relative md:row-[1/2] px-4 md:px-0 gap-x-2 flex items-center justify-between">
            <div className="border md:border-0 px-4 select-none md:w-full w-1/2 h-full flex text-sm lg:text-[1rem] items-center rounded-lg bg-white justify-start gap-x-4 text-gray-400">
              ...
              <div ref={sort} className='h-full fixed duration-300 transition-all md:border-0 overflow-hidden md:relative bg-white w-full border-t md:w-auto max-h-0 md:h-full md:max-h-full right-0 flex-col md:flex-row bottom-0 z-40 md:z-10  md:flex items-center justify-center gap-4'>
                <span onClick={(e) => sortHandler(e)} name="sort" value="Bestselling" className={`${filter.sort === "پرفروش ترین" && "font-bold text-slate-900"} text-center py-5 cursor-pointer w-full md:w-auto block`}>پرفروش ترین</span>
                <span onClick={(e) => sortHandler(e)} name="sort" value="MostVisited" className={`${filter.sort === "پربازدید ترین" && "font-bold text-slate-900"} text-center py-5 cursor-pointer w-full md:w-auto block`}>پربازدید ترین</span>
                <span onClick={(e) => sortHandler(e)} name="sort" value="MostExpensive" className={`${filter.sort === "گرانترین" && "font-bold text-slate-900"} text-center py-5 cursor-pointer w-full md:w-auto block`}>گرانترین</span>
                <span onClick={(e) => sortHandler(e)} name="sort" value="Ceapest" className={`${filter.sort === "ارزان ترین" && "font-bold text-slate-900"} text-center py-5 cursor-pointer w-full md:w-auto block`}>ارزان ترین</span>
              </div>
