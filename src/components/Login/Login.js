  ...
  return (
    <section className="fixed w-screen h-screen z-50 top-0 right-0 bg-white flex flex-col items-center justify-center">
      <form className="w-full max-w-[400px] px-8 md:border rounded-lg flex flex-col items-center justify-center">
        ...
        <h2 className="font-sans text-2xl text-bold w-full text-right mb-9">ورود | ثبت نام</h2>
        <div className="gap-y-1.5 font-sans text-sm text-slate-600 w-full flex flex-col items-start justify-center">
          <span>سلام!</span>
          <p>لطفا شماره موبایل یا ایمیل خود را وارد کنید</p>
        </div>
        <input type={"text"} className="outline-none border rounded-lg w-full p-3 mt-5 mb-8" />
        <button className="w-full p-4 bg-red-500 text-center rounded-lg font-sans text-white">ورود</button>
        <p className="font-sans whitespace-pre-wrap text-sm text-slate-800 flex items-center justify-center mt-6 mb-10">
