const Input = ({ formik, lable, name, type = "text", children }) => {
  return (
    <div className="flex flex-col items-start gap-y-2 mt-5 w-full">
      <div className={`${!formik.errors[name] && formik.touched[name] && "border-blue-600"} bg-white w-full px-2 py-1 flex items-center justify-start rounded-lg border ${formik.errors[name] && formik.touched[name] && "border-red-500"}`}>
        {children}
        <input
          type={type}
          name={name}
