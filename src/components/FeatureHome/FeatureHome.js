...
const FeatureHome = () => {
  ...
  const handleMorePage = (dispatch) => {
    ...
    if (dispatch === "open") {
      ...
      isMobile ? morePage.current.classList.replace("top-full", "top-0")
        : morePage.current.classList.replace("top-full", "top-1/4")
    } else {
      backdrop.current.classList.replace("top-0", "top-full")
      isMobile ? morePage.current.classList.replace("top-0", "top-full")
        : morePage.current.classList.replace("top-1/4", "top-full")
    }
  }
